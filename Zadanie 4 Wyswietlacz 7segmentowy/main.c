...
ISR(TIMER0_COMP_vect){
	...
	//shedule();
	if ((++delay % 100) == 0)	//wygaszenie wyœwietlacza i zmiana wyswietlanego elementu co 1s
	{
		if( (++segment[0] %10) == 0){			//wyznaczamy kolejne wartoœci dla poszczególnych segmentow
			if((++segment[1] %10) == 0){
				if((++segment[2] %10) == 0){
					if((++segment[3] %10) == 0){
						segment[0] = segment[1] = segment[2] = segment[3] = 0;	//po osi¹gniêciu 9999 zerujemy wszystko
					}
				}
			}
		}
		...
		if( (delay % 50) == 0){	//zmiana krazacego zera co 0.5s nie jesteœmy w stanie bez p³ytki dopasowaæ szybkoœci zmian wyœwietlaczy aby zmianialo sie 'plynnie'
			if( (kolumn<<1) <= 8 ){	//przesuniêcia bitowe *2 wtedy mamy do dyspozycji tylko PINy 1,2,4,8 aby nie zaswiecac dwoch wyœwietlaczy na raz
				kolumn<<1;
				kolumna = ~kolumn;
				pokaz_segment++;
			}
